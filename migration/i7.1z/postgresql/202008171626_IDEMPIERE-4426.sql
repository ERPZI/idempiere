...
DROP VIEW zi_rv_openitem;
DROP VIEW zi_rv_openitemtodate;
...
;
CREATE OR REPLACE VIEW adempiere.zi_rv_openitem
 AS
 SELECT i.ad_org_id,
    i.ad_client_id,
    i.documentno,
    ...
    bpe.zi_salesrep1_id,
    bpe.zi_salesrep2_id,
    ( SELECT sum(lco_invoicewithholding.taxamt) AS sum
           FROM lco_invoicewithholding
          WHERE lco_invoicewithholding.c_invoice_id = i.c_invoice_id) AS wht_taxamt
   FROM rv_c_invoice i
     JOIN c_paymentterm p ON i.c_paymentterm_id = p.c_paymentterm_id
     LEFT JOIN c_bpartner bp ON i.c_bpartner_id = bp.c_bpartner_id
     LEFT JOIN zi_c_bp_extension bpe ON bp.c_bpartner_id = bpe.c_bpartner_id AND bp.ad_org_id = bpe.ad_org_id AND bpe.isactive = 'Y'::bpchar
...
ALTER TABLE adempiere.zi_rv_openitem
    OWNER TO adempiere;

CREATE OR REPLACE VIEW adempiere.zi_rv_openitemtodate
 AS
 SELECT i.ad_org_id,
    i.ad_client_id,
    i.documentno,
...
ALTER TABLE adempiere.zi_rv_openitemtodate
    OWNER TO adempiere;
